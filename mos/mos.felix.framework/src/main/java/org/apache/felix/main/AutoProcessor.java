...
public class AutoProcessor {
	...
	/**
	 * This bundles will not be started automatically
	 */
//	private static final String[] filterBundles = new String[] { "mos.felix.framework" };
		...
		if (actionList.size() > 0) {
			...
			if (configMap.get(AUTO_DEPLOY_STARTLEVEL_PROPERY) != null) {
				try {
					startLevel = Integer.parseInt(configMap.get(
							AUTO_DEPLOY_STARTLEVEL_PROPERY).toString());
			...
			for (int i = 0; i < jarList.size(); i++) {
				...
				try {
					...
					if ((b == null)
							&& actionList.contains(AUTO_DEPLOY_INSTALL_VALUE)) {
					...
					// if the 'update' action is present.
							...
							&& actionList.contains(AUTO_DEPLOY_UPDATE_VALUE)) {
						b.update();
					}
			...
			if (actionList.contains(AUTO_DEPLOY_UNINSTALL_VALUE)) {
				for (Iterator it = installedBundleMap.entrySet().iterator(); it
						.hasNext();) {
